  ...
  await firebase
    ...
    .then(async (snap) => {
      let promises = snap.docs.map(async (d) => {
        ...
        let industries = [];
        if (data.industries) {
          let promises = data.industries.map(
            async (ref) => await ref.get().then((res) => res.data())
          );
...
export default ({ majors, schools, companies, tools }) => {
  ...
  const onFill = () => {
    form.setFieldsValue({
      ...
      tools: [{ name: "IntelliJ IDEA", usage: "often" }],
      expectations: 2,
      impact: 2,
      prerequisites: 2,
      work_time: 2,
  ...
  const onNext = () => {
    form
      .validateFields()
      .then((res) => {
        router.push("/submit/[section]", `/submit/${sectionTitles[formIndex + 1]}`, {
          shallow: true,
        });
      })
      .catch((err) => {
  ...
  const status = (formIndex, index) =>
    formIndex === index ? "process" : formIndex > index ? "finish" : "wait";
  ...
  const onFinish = async (values) => {
    ...
    review.majors = review.majors.map((major) =>
      firebase.firestore().collection("majors").doc(major)
    );
    ...
    let promises = review.tools.map(async (tool) => {
      ...
      if (foundTool) {
        return {
          tool: firebase.firestore().collection("tools").doc(foundTool.id),
          usage: tool.usage,
        };
      } else {
        let newTool;
  ...
  return (
    <Main className="submit">
      ...
      <Form
        ...
        className="form"
      >
        {formIndex >= formSections.length - 1 ? (
          <div>
            <div style={{ display: showAll ? "block" : "none" }}>
              {formSections.slice(0, -1).map((section, i) => (
        ...
        <Form.Item style={{ textAlign: "right" }}>
          ...
          {formIndex === sectionTitles.length - 1 ? (
            <span>
              ...
              <Button type="link" style={{ marginRight: "5px" }} onClick={() => setShowAll(true)}>
                Review all answers
              </Button>
            </span>
          ) : (
...
const AboutYou = ({ majorOptions, schoolOptions }) => (
  <div className="about-you">
    ...
    <Form.Item label="School" rules={[{ required: true }]} name="school">
      <Select
        placeholder="School"
    ...
    <Form.Item label="Major(s)" rules={[{ required: true }]} name={["majors"]}>
      <Select
        allowClear
        mode="multiple"
        placeholder="Major(s)"
    ...
    <Form.Item rules={[{ required: true }]} label="Other studies" name={["other_studies"]}>
...
const InternshipDetails = ({ companyOptions }) => (
  <div className="internship-details">
    ...
    <Form.Item name={["location"]} label="Internship Location" rules={[{ required: true }]}>
    ...
    <Form.Item rules={[{ required: true }]} label="Position title" name={["position"]}>
      <Input placeholder="Position title" />
    </Form.Item>

    <Form.Item rules={[{ required: true }]} label="Deptartment/Team" name={["team"]}>
      <Input placeholder="Deptartment/Team" />
    </Form.Item>

    <Form.Item name="terms" label="Term(s) employed" rules={[{ required: true }]}>
      <Select allowClear mode="multiple" placeholder="Terms employed" tokenSeparators={[","]}>
        {terms}
      </Select>
    </Form.Item>

    <Form.Item name="pay" label="Compensation" rules={[{ required: true }]}>
      <PayInput />
    </Form.Item>

    <Form.Item label="Housing stipend" name={["housing_stipend"]}>
    ...
    <Form.Item
      ...
      shouldUpdate={(prevValues, currentValues) =>
        prevValues.full_time_offer !== currentValues.full_time_offer
      }
    >
      {({ getFieldValue }) => {
        return (
          !!(getFieldValue("full_time_offer") && getFieldValue("full_time_offer") > 1) && (
            <Form.Item
              name="would_accept_full_time"
              label="Would you accept a full-time offer?"
              rules={[{ required: true }]}
            >
              <Radio.Group>
                <Radio style={verticalStyle} value={0}>
...
const InternshipExperience = ({ tools }) => (
  <div className="internship-experience">
    ...
    <Form.Item name="impact" label="Impact of your work" rules={[{ required: true }]}>
      <Radio.Group>
        <Radio style={verticalStyle} value={0}>
          No impact (busy-work)
        </Radio>
    ...
    <Form.Item name="tools" rules={[{ required: true }]} label="Tools used">
    ...
    <Form.Item name="description" label="Describe your internship." rules={[{ required: true }]}>
      <Input.TextArea
