...
import { Row, Col, AutoComplete, Input, Select, Button, Form, DatePicker, Tag, Divider } from 'antd';
const { Option } = Select
import { SearchOutlined, ArrowDownOutlined, EnvironmentOutlined, FilterOutlined } from '@ant-design/icons';
...
export async function getServerSideProps({ query }) {
  ...
  await firebase.firestore().collection('reviews').where('visible', '==', true).get().then(async snap => {
    let promises = snap.docs.map(async d => {
      let data = d.data()
      ...
      await firebase.firestore().collection('users_test').doc(data.user.id).get().then(res => {
        user = res.data()
      })
...
const Main = styled.div`
  ...
  .search .ant-input:focus, .ant-input-focused, .ant-input-affix-wrapper:focus, .ant-input-affix-wrapper-focused {
...
const SearchItem = styled(({ prompt, search, className }: { prompt: string, search: string, className?: string }) => (
...
export default ({ majors, schools, companies, reviews, tools }) => {
  ...
  const majorOptions = majors?.map((major, i) => <Option key={i} value={major.name}>{major.name}</Option>)
  const schoolOptions = schools?.map((school, i) => <Option key={i} value={school.name}>{school.name}</Option>)
  const companyOptions = companies?.map((company, i) => <Option key={i} value={company.name}>{company.name}</Option>)
  const toolOptions = tools?.map((tool, i) => <Option key={i} value={tool.name}>{tool.name}</Option>)  
      ...
      <Form layout="vertical" form={form} onValuesChange={onFormChange} initialValues={initialValues}>
        <div className="search-bar">
          <Row>
            ...
            <Col className="menu-row" {...{ xs: 0, sm: 0, md: 20, lg: 20, xl: 21, xxl: 22 }}>
              <Form.Item noStyle shouldUpdate={(prevValues, currentValues) => prevValues.search !== currentValues.search}>
                {({ getFieldValue }) => (
        ...
        <div className="content">
          ...
          <div className="results">
            {reviews.map((review, i) => <div key={i} className="review-container"><ReviewCard {...review} /></div>)}
          </div>
        </div>
      </Form>
    </Main>
...
const Filter = ({ majorOptions, schoolOptions, companyOptions, toolOptions }) => {
  
  return (
    <FilterMain className="filter">
